...
var widthScale = d3.scaleLinear().domain([0, 5000]).range([0, width]);
var yPositionScale = d3.scaleBand().range([height, 0]).padding(0.25); // var line = d3
//   .line()
//   .x(d => xPositionScale(d.year))
//   .y(d => yPositionScale(d.cumulativePoints))
//   .curve(d3.curveStepAfter)

d3.csv(require('./data/top_solicitantes.csv')).then(ready).catch(function (err) {
...
function ready(datapoints) {
  ...
  var xAxis = d3.axisBottom(widthScale).tickValues([0, 1000, 2000, 3000, 4000, 5000]);
  svg.append('g').attr('class', 'axis x-axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
  svg.append('text').attr('class', 'title-text').attr('font-size', '20').attr('text-anchor', 'middle').text('Number of asylum requests per year').attr('x', width / 2).attr('y', -20).attr('dx', 20);
  svg.selectAll('.countries').data(datapoints).enter().append('rect').attr('class', 'countries').attr('width', 0).attr('fill', '#cb181d').attr('height', yPositionScale.bandwidth()).attr('x', 0).attr('y', function (d) {
    return yPositionScale(d.COUNTRY);
  });
  d3.select('#blank-graph').on('stepin', function () {
    svg.selectAll('.countries').transition().attr('width', 0);
    d3.select('#first-graph').on('stepin', function () {
      svg.selectAll('.countries').transition().attr('width', function (d) {
        return widthScale(d.SOLICITANTES13);
      });
      d3.select('#second-graph').on('stepin', function () {
        svg.selectAll('.countries').transition().attr('width', function (d) {
          return widthScale(d.SOLICITANTES14);
        });
        d3.select('#third-graph').on('stepin', function () {
          svg.selectAll('.countries').transition().attr('width', function (d) {
            return widthScale(d.SOLICITANTES15);
          });
          d3.select('#fourth-graph').on('stepin', function () {
            svg.selectAll('.countries').transition().attr('width', function (d) {
              return widthScale(d.SOLICITANTES16);
            });
            d3.select('#fifth-graph').on('stepin', function () {
              svg.selectAll('.countries').transition().attr('width', function (d) {
...
//   render()
},{"d3":"../node_modules/d3/index.js","./data/top_solicitantes.csv":"data/top_solicitantes.csv"}],"../../../../../.npm-global/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/01-chart.88ac8497.map